...
Set to mpv error string describing the approximate reason why playback failed. Unset if no error known.
...
Sent when the player quits, and the script should terminate. Normally handled automatically. See [Details on the script initialization and lifecycle](#details-on-the-script-initialization-and-lifecycle).
