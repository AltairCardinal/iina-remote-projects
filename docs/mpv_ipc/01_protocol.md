...
> echo '{ "command": \["get\_property", "playback-time"\] }' | socat - /tmp/mpvsocket
...
{ "command": \["command\_name", "param1", "param2", ...\] }
...
where event\_name is the name of the event. Additional event-specific fields can also be present. See [List of events](#list-of-events) for a list of all supported events.
...
{ "command": \["get\_property", "time-pos"\], "request\_id": 100 }

Would generate this response:

{ "error": "success", "data": 1.468135, "request\_id": 100 }
...
{ "command": \["screenshot"\], "request\_id": 123, "async": true }

And this is the completion:

{"request\_id":123,"error":"success","data":null}
...
Currently, only "proper" commands (as listed by [List of Input Commands](#list-of-input-commands)) support named arguments.

## IPC-specific Commands

In addition to the commands described in [List of Input Commands](#list-of-input-commands), a few extra commands can also be used as part of the protocol:
...
Set the given property to the given value. See [Properties](#properties) for more information about properties.
...
You can put .run file extension in the mpv scripts directory in its config directory (see the [FILES](#files) section for details), or load them through other means (see [Script location](#script-location)). These scripts are simply executed with the OS native mechanism (as if you ran them in the shell). They must have a proper shebang and have the executable bit set.
